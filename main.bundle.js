...
var createParticule = function createParticule(_ref6) {
  ...
  p.alpha = 0;
  p.delay = 0;
  p.fadingStart = p.xPos * 0.5 + p.yPos * 0.5; // p.colour = getColour(p.xPos/ftue.iconW, p.yPos/ftue.iconW);

  p.colour = 0;
  p.outroStart = 0; // flags

  p.state = 'fadeIn';
  p.remove = false;
  p.bigger = false;
  p.colourChange = false; // p.tweening = false;
